...
public class Adaptador_favoritos extends BaseAdapter {
    ...
    public View getView(int position, View v, ViewGroup parent) {
        ...
        button2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                fragment_favoritos ff =new fragment_favoritos();
                conectar.eliminar(db_escribir ,datosItem.getNombre());
                for(Entidad lista: ff.getListaItems()){
                    System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
                    System.out.println(lista.getNombre());
                    System.out.println(datosItem.getNombre());
                    if(lista.getNombre()==datosItem.getNombre()){
                        ff.getListaItems().remove(lista);
                        Toast.makeText(view.getContext(), "Eliminado de los favoritos", Toast.LENGTH_LONG).show();
                    }
                }
